...
<div id="content" class="site-content page-cart cl">
						...
						<td class="product-quantity">
							<?php
								if ( $_product->is_sold_individually() ) {
									...
									$product_quantity = sprintf( '<div class="quantity"><input type="number" name="cart[%s][qty]" step="%s" min="%s" max="%s" value="%s" title="' . _x( 'Qty', 'Product quantity input tooltip', 'woocommerce' ) . '" class="input-text qty text" maxlength="12" /></div>', $cart_item_key, $step, $min, $max, esc_attr( $values['quantity'] ) );
								}
			...
			<td colspan="6" class="actions">
				...
				<?php if ( $woocommerce->cart->coupons_enabled() ) { ?>
					<div class="coupon">
						...
						<label for="coupon_code"><?php _e( 'Coupon', 'woocommerce' ); ?>:</label> <input name="coupon_code" class="input-text" id="coupon_code" value="" placeholder="Enter Coupon Code Here" /> <input type="submit" class="button" name="apply_coupon" value="<?php _e( 'Apply Coupon', 'woocommerce' ); ?>" />
